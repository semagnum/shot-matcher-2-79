...
def draw_layer(itself, context, sm_layer, sm_layer_type):
    ...
    if sm_layer_type == 'video':
        ...
        if itself.bl_space_type == 'CLIP_EDITOR':
            box.operator(SM_OT_set_selected.bl_idname, text='Use Current Clip', icon='FILE_MOVIE').space_type = 'CLIP_EDITOR'
    else:
        ...
        if itself.bl_space_type == 'IMAGE_EDITOR':
            box.operator(SM_OT_set_selected.bl_idname, text='Use Current Image', icon='FILE_IMAGE').space_type = 'IMAGE_EDITOR'
